...
export default function TodayTasks() {
  ...
  const handleFileSelect = (itemId: string, files: FileList | null) => {
    ...
    toast.success(`已選取 ${newFiles.length} 個檔案`);
  };
    ...
    <div className="flex flex-col min-h-full bg-background">
      <div className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-border shadow-elegant">
        <div className="flex items-center justify-between px-4 pt-4 pb-2">
          <button onClick={() => setCurrentDate(d => subDays(d, 1))} className="w-9 h-9 rounded-full flex items-center justify-center hover:bg-muted transition-colors">
          ...
          <button onClick={() => setCurrentDate(d => addDays(d, 1))} className="w-9 h-9 rounded-full flex items-center justify-center hover:bg-muted transition-colors">
                ...
                <div className="px-4 pb-4 space-y-3 border-t border-border/40 pt-3">
                  <div className="flex gap-2">
                    {[
                      ...
                      { icon: Paperclip, label: "選檔案", opts: { accept: ".pdf,.doc,.docx", multiple: true } },
                    ].map(({ icon: Icon, label, opts }) => (
                      <button key={label} onClick={() => openFilePicker(task.itemId, opts)}
                        className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-slate-50 text-slate-700 text-sm font-medium hover:bg-slate-100 transition-colors">
                    ...
                    <div className="space-y-2">
                      {task.files.map(file => (
                        <div key={file.id} className="flex items-center gap-2 bg-muted/50 rounded-lg px-3 py-2">
                          ...
                          <button className="p-1 hover:bg-muted rounded" onClick={() => window.open(file.url, "_blank")}><Eye className="w-3.5 h-3.5 text-muted-foreground" /></button>
                          <button className="p-1 hover:bg-red-50 rounded" onClick={() => removeFile(task.itemId, file.id)}><Trash2 className="w-3.5 h-3.5 text-red-400" /></button>
                        </div>
                      ))}
                    </div>
                  )}
                  {showNoteFor.has(task.itemId) ? (
                    <div className="relative">
                      <input type="text" placeholder="輸入備註..." value={task.note}
                        ...
                        className="w-full text-sm border border-border rounded-lg px-3 py-2 pr-8 focus:outline-none focus:ring-2 focus:ring-blue-300" />
                      <button className="absolute right-2 top-1/2 -translate-y-1/2" onClick={() => setShowNoteFor(prev => { const n = new Set(prev); n.delete(task.itemId); return n; })}>
                  ...
                  <div className={cn("flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all",
      ...
      {showConfirmDialog && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-[430px] bg-white rounded-t-3xl p-6">
            ...
            <p className="text-sm text-muted-foreground mb-6">確定送出 {format(currentDate, "M/d", { locale: zhTW })} 的 {completedCount} 項工作資料？</p>
            <div className="flex gap-3">
              <Button variant="outline" className="flex-1" onClick={() => setShowConfirmDialog(false)}>取消</Button>
              <Button className="flex-1 bg-blue-700 hover:bg-blue-800" onClick={handleSubmit} disabled={isSubmitting}>
      ...
      <div className="sticky bottom-0 px-4 py-3 bg-white/95 backdrop-blur-md border-t border-border pb-safe shadow-elegant">
        {isSubmitted ? (
          <Button className="w-full bg-slate-600 hover:bg-slate-700 h-12 text-base font-medium rounded-xl" disabled>查看已送出內容</Button>
        ) : (
          <Button
            className={cn("w-full h-12 text-base font-medium rounded-xl transition-all",
              hasAnyCompleted ? "bg-blue-700 hover:bg-blue-800 shadow-elegant-md" : "bg-muted text-muted-foreground cursor-not-allowed")}
