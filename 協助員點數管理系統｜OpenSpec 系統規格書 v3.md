...
| 版本差異 | 本版（v3.0）為 **統整定稿版**，包含單一檔案架構轉換、中文欄位更新。最新維護（v3.1）則針對了出勤計畫表（新增表格介面即時下拉選單與預估點數功能）、月報傳遞修正及無縫的 Drive ID 容錯機制進行了重點強化設計。 |
...
### 4.2.0 出勤計畫表 `/attendance`（Worker 視角）

- **雙視圖系統**：提供傳統月曆（Calendar）與出勤計畫表（Table）雙模式切換。
- **即時出勤編輯**：在出勤計畫表模式下，針對未鎖定的日期，可透過 `<select>` 下拉選單直接修改上午/下午狀態（如：出勤、特休、病假...等），並即時背景同步更新至資料庫。
- **本月預估點數**：在畫面頂端直接匯總估算（計算有效工作日 × A1類每日點數 + B類每月固定點數 + 預設 C類「平」點數），幫助協助員隨時掌握進度。
