  ...
  const headers = {
    ...
    [SHEET_NAMES.WORKERS]: [
      ["工號", "姓名", "帳號類型", "Email", "部門", "服務區域", "協助員類型", "到職日期",
       "離職日期", "狀態", "小計經驗天數", "備註", "建立時間", "更新時間", "密碼雜湊"]
    ],
...
// ============================================================
// Web App 入口：doGet（統一路由，涵蓋 Init.gs 與 API.gs 所有 action）
// 注意：GAS 專案中只能有一個生效的 doGet，本檔案為主要入口
// ============================================================
function doGet(e) {
  ...
  try {
    ...
    switch (action) {
      ...
      case "getPointDefinitions":
        ...
        break;
      case "getAttendance":
        data = getAttendance(e.parameter.workerId, e.parameter.month || e.parameter.yearMonth);
        break;
      case "getDailyPoints":
        data = getDailyPoints(e.parameter.workerId, e.parameter.date);
        break;
      case "getMonthlyPoints":
        data = getMonthlyPoints(e.parameter.workerId, e.parameter.month || e.parameter.yearMonth);
        break;
      case "getConfig":
  ...
  try {
    ...
    switch (action) {
      ...
      case "submitMonthlyReport":
        data = submitMonthlyReport(body.data || body);
        break;
      case "reviewItem":
      case "reviewMonthlyReport":
        data = reviewItem(body.data || body);
        break;
      case "saveFileIndex":
        data = saveFileIndex(body.data || body);
        break;
      case "upsertWorker":
        data = upsertWorker(body.data || body.worker || body);
        break;
      ...
      case "setWorkerPassword":
        data = setWorkerPassword(body.workerId, body.passwordHash);
        break;
      default:
        data = { success: false, error: "未知的 action：" + action };
    }
...
// 登入驗證：以 Email 查詢人員名冊，回傳個人資料
function getMyProfile(callerEmail) {
  if (!callerEmail) {
    return { success: false, error: "缺少 callerEmail 參數" };
  }
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(SHEET_NAMES.WORKERS);
  const data = sheet.getDataRange().getValues();
  if (data.length < 2) return { success: false, error: "人員名冊為空" };
  const headers = data[0];
  const emailCol = headers.indexOf("Email");
  if (emailCol === -1) return { success: false, error: "人員名冊缺少 Email 欄位" };

  for (let i = 1; i < data.length; i++) {
    if (String(data[i][emailCol]).toLowerCase() === String(callerEmail).toLowerCase()) {
      const worker = {};
      headers.forEach(function(h, j) { worker[h] = data[i][j]; });
...
/**
 * 以 email + SHA-256(密碼) 比對人員名冊「密碼雜湊」欄
 * 若帳號存在但 密碼雜湊 欄為空 → 提示改用 Google 登入
 */
function passwordLogin(email, passwordHash) {
  if (!email || !passwordHash) {
    return { success: false, error: "缺少 email 或 passwordHash 參數" };
  }
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(SHEET_NAMES.WORKERS);
  const data = sheet.getDataRange().getValues();
  ...
  const headers = data[0];
  const emailCol    = headers.indexOf("Email");
  const hashCol     = headers.indexOf("密碼雜湊");
  const statusCol   = headers.indexOf("狀態");

  if (emailCol === -1) return { success: false, error: "人員名冊缺少 Email 欄位" };

  for (let i = 1; i < data.length; i++) {
    if (String(data[i][emailCol]).toLowerCase() !== String(email).toLowerCase()) continue;

    // 帳號存在
    const status = statusCol >= 0 ? String(data[i][statusCol]) : "";
    if (status === "離職") return { success: false, error: "此帳號已停用，請聯絡管理員。" };

    const storedHash = hashCol >= 0 ? String(data[i][hashCol]) : "";
    if (!storedHash) {
      return { success: false, error: "此帳號尚未設定密碼，請改用 Google 帳號登入。" };
    }
...
/**
 * 管理員設定人員密碼（存 SHA-256 hash）
 * 呼叫方式：POST { action: "setWorkerPassword", workerId, passwordHash }
 */
function setWorkerPassword(workerId, passwordHash) {
  if (!workerId || !passwordHash) {
    return { success: false, error: "缺少 workerId 或 passwordHash 參數" };
  }
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(SHEET_NAMES.WORKERS);
  const data = sheet.getDataRange().getValues();
  const headers = data[0];
  const idCol   = headers.indexOf("工號");
  const hashCol = headers.indexOf("密碼雜湊");

  if (hashCol === -1) {
    return { success: false, error: "人員名冊缺少「密碼雜湊」欄位，請先執行 addPasswordHashColumn()" };
  }

  for (let i = 1; i < data.length; i++) {
    if (String(data[i][idCol]) === String(workerId)) {
      sheet.getRange(i + 1, hashCol + 1).setValue(passwordHash);
      return { success: true };
    }
  }
  return { success: false, error: "查無工號：" + workerId };
}

/**
 * 一次性遷移：在已存在的人員名冊末尾新增「密碼雜湊」欄位
 * 在 GAS 編輯器中手動執行一次即可
 */
function addPasswordHashColumn() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(SHEET_NAMES.WORKERS);
  const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
  if (headers.includes("密碼雜湊")) {
    Logger.log("「密碼雜湊」欄位已存在，略過");
    SpreadsheetApp.getUi().alert("「密碼雜湊」欄位已存在，無需再次新增。");
    return;
  }
  const newCol = sheet.getLastColumn() + 1;
  sheet.getRange(1, newCol).setValue("密碼雜湊");
  sheet.getRange(1, newCol).setBackground("#1e3a5f").setFontColor("#ffffff").setFontWeight("bold");
  sheet.autoResizeColumn(newCol);
  Logger.log("已新增「密碼雜湊」欄位（第 " + newCol + " 欄）");
  SpreadsheetApp.getUi().alert("✅ 已新增「密碼雜湊」欄位（第 " + newCol + " 欄）。\n\n請至管理介面為各人員設定密碼。");
}
